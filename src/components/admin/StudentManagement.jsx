...
const StudentManagement = () => {
  const [students, setStudents] = useState(users.filter(u => u.role.toLowerCase() === "student"));
  ...
  return (
    <div className={styles.adminContainer}>
            ...
            {filteredStudents.map((student) => (
              <TableRow key={student.email}>
                ...
                <TableCell>
                  ...
                  <IconButton onClick={() => handleDeleteClick(student)}><Delete color="error" /></IconButton>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <TextField margin="dense" label="Tên" name="name" value={editingStudent?.name || ""} onChange={handleChange} fullWidth />
          <TextField margin="dense" label="Mật khẩu" name="password" value={editingStudent?.password || ""} onChange={handleChange} fullWidth />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          <p>Nhập lại tên sinh viên để xác nhận: <strong>{targetStudent?.name}</strong></p>
          <TextField
            fullWidth
            autoFocus
            value={confirmName}
