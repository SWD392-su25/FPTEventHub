...
const LecturerManagement = () => {
  const [lecturers, setLecturers] = useState(users.filter(u => u.role.toLowerCase() === "lecture"));
  ...
  return (
    <div className={styles.adminContainer}>
          ...
          <TableBody>
            {filteredLecturers.map((lecturer) => (
              <TableRow key={lecturer.email}>
                ...
                <TableCell>
                  <IconButton onClick={() => handleEdit(lecturer)}><Edit /></IconButton>
                  <IconButton onClick={() => handleDeleteClick(lecturer)}><Delete color="error" /></IconButton>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <TextField margin="dense" label="Tên" name="name" value={editingLecturer?.name || ""} onChange={handleChange} fullWidth />
          <TextField margin="dense" label="Mật khẩu" name="password" value={editingLecturer?.password || ""} onChange={handleChange} fullWidth />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          <p>Nhập lại tên giảng viên để xác nhận: <strong>{targetLecturer?.name}</strong></p>
          <TextField
            fullWidth
            autoFocus
            value={confirmName}
